      ...
      <div className="flex flex-wrap items-center gap-3">
          ...
          {([
            ...
            <button key={id} type="button" onClick={() => setFilter(id)}
              className={clsx('rounded px-3 py-1 text-xs font-semibold transition',
                filter === id ? 'bg-signal/15 text-white' : 'text-chalk-dim hover:text-chalk')}>
              {label}
            </button>
          ))}
        </div>
        ...
        <div className="flex flex-1 items-center justify-end gap-3">
          <span className="text-xs text-chalk-faint">
            {eventName && <span className="mr-2 text-chalk-dim">{eventName}</span>}
            {scouted.reduce((n, s) => n + s.matchesPlayed, 0)} robot-matches
          </span>
          {roster.length > 0 && (
            <div className="flex items-center gap-2">
              <div className="h-1.5 w-24 overflow-hidden rounded-full bg-deck-900">
        ...
        <table className="w-full min-w-[900px] border-collapse text-sm">
          <thead className="border-b border-deck-500 bg-deck-900">
            <tr>
              ...
              <th className="px-3 py-2.5 text-left font-display text-[13px] font-600 text-chalk-dim">
                Matches
              </th>
          ...
          <tbody>
            {rows.map((s) => {
              ...
              return (
                <tr key={s.teamNumber}
                  className={clsx('border-b border-deck-600 transition hover:bg-deck-600',
                    !hasData && 'opacity-45')}>
                  <td className="px-3 py-2">
                    <Link to={`/analysis/${s.teamNumber}`} className="group block">
                      <span className="font-mono font-bold text-chalk group-hover:text-chalk">
                        {s.teamNumber}
                      </span>
                      {teamInfo(event, s.teamNumber)?.nickname && (
                        <span className="block max-w-[180px] truncate text-[11px] text-chalk-dim">
                          {teamInfo(event, s.teamNumber)!.nickname}
                        </span>
                      )}
                    </Link>
                  </td>
                  <td className="px-3 py-2 tabular-nums text-chalk-dim">
                    {hasData ? s.matchesPlayed : <span className="text-chalk-faint">not scouted</span>}
                  </td>

                  {game.keyMetrics.map((m) => {
                    if (!hasData) return <td key={m.id} className="px-3 py-2 text-chalk-faint">—</td>
                  ...
                  <td className="px-3 py-2">
                    {!hasData ? <span className="text-chalk-faint">—</span> : (
                      <span className={clsx('tabular-nums',
                        s.breakdownRate > 0.25 ? 'text-alliance-red'
                          : s.breakdownRate > 0 ? 'text-signal' : 'text-chalk-faint')}>
  ...
  return (
    <th className="px-3 py-2.5 text-left">
      <button type="button" onClick={onClick}
        className={clsx('font-display text-[13px] font-600 transition',
